...
void Mesh::Create(GraphicsDevice* pGraphicsDevice)
{
	...
	auto hr = m_pDevice->GetDevice()->CreateCommittedResource(&heapProp, D3D12_HEAP_FLAG_NONE, &resDesc,
		D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&m_pVBuffer));
...

	{
		...
		if (FAILED(hr))assert(0 && "インデックスバッファ作成失敗");
