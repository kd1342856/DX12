...
bool GraphicsDevice::Init(HWND  hWnd, int w, int h)
{
	if (!CreateFactory())
	{
		assert(0 && "Factory creation failed");
		return false;
	}
#ifdef _DEBUG
	EnableDebugLayer();
	...
	if (!CreateDevice())
	{
		assert(0 && "Device creation failed");
		return false;
	}
	if (!CreateCommandList())
	{
		assert(0 && "CommandList creation failed");
		return false;
	}
	if (!CreateSwapChain(hWnd, w, h))
	{
		assert(0 && "SwapChain creation failed");
		return false;
	}
	m_pRTVHeap = std::make_unique<RTVHeap>();
	if (!m_pRTVHeap->Create(this, HeapType::RTV, 100))
	{
		assert(0 && "RTV Heap creation failed");
		return false;
	}
	...
	{
		assert(0 && "CBVSRVUAV Heap creation failed");
		return false;
	}
	...
	{
		assert(0 && "DSV Heap creation failed");
		return false;
	}
	m_upDepthStencil = std::make_unique<DepthStencil>();
	if(!m_upDepthStencil->Create(this, Math::Vector2(w, h)))
	{
		assert(0 && "DepthStencil creation failed");
		return false;
	}
	if (!CreateSwapChainRTV())
	{
		assert(0 && "SwapChain RTV creation failed");
		return false;
	}
	if (!CreateFence())
	{
		assert(0 && "Fence creation failed");
		return false;
	}
	m_fenceEvent = CreateEvent(nullptr, false, false, nullptr);
	if (!m_fenceEvent)
	{
		assert(0 && "Event error, application shutdown");
		return false;
	}
	CreateDefaultTextures();
	if (!InitImGui())
	{
		assert(0 && "ImGui initialization failed");
		return false;
	}
	return true;
}
...
// =============================================
// GraphicsDevice.cpp (ImGui統合部分)
// =============================================

bool GraphicsDevice::InitImGui()
{
	// ImGui用のSRVデスクリプタヒープを作成（フォントテクスチャ用）
	...
	// Win32バックエンドの初期化はWin32Windowが持つhWndが必要なので
	// ここでは ImGui_ImplDX12のみ初期化。Win32の初期化はmain/Applicationに任せる。
	...
	{
		// 今はフォント用に1スロットだけ固定で割り当て
		auto& dev = GraphicsDevice::Instance();
		*out_cpu = dev.m_upImGuiSRVHeap->GetCPUDescriptorHandleForHeapStart();
		*out_gpu = dev.m_upImGuiSRVHeap->GetGPUDescriptorHandleForHeapStart();
	};
...
void GraphicsDevice::ShutdownImGui()
{
	...
	// Win32バックエンド解放（DestroyContext前に必ず呼ぶ）
...
void GraphicsDevice::SetRenderTarget(RenderTarget* pRT)
{
	...
	// リソースバリアをRENDER_TARGETに変更
	SetResourceBarrier(pRT->GetResource(), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_RENDER_TARGET);
