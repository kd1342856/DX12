...
bool GraphicsDevice::Init(HWND  hWnd, int w, int h)
{
	...
	{
		assert(0 && "CBV/SRV/UAVヒープの作成に失敗しました");
		return false;
	}
	...
	{
		assert(0 && "スワップチェーンのRTV作成に失敗しました");
		return false;
	}
	if (!CreateFence())
	{
	...
	if (!m_fenceEvent)
	{
		assert(0 && "イベントエラーが発生しました。アプリケーションを終了します");
		return false;
	}
