...
bool GraphicsDevice::Init(HWND  hWnd, int w, int h)
{
	...
	{
		assert(0 && "Failed to create CommandList");
		return false;
	}
	if (!CreateSwapChain(hWnd, w, h))
	{
		assert(0 && "Failed to create SwapChain");
		return false;
	}
	m_pRTVHeap = std::make_unique<RTVHeap>();
	if (!m_pRTVHeap->Create(this, HeapType::RTV, 100))
	{
		assert(0 && "Failed to create RTV Heap");
		return false;
	}
	...
	{
		assert(0 && "Failed to create CBV/SRV/UAV Heap");
		return false;
	}
	...
	{
		assert(0 && "Failed to create DSV Heap");
		return false;
	}
	...
	{
		assert(0 && "Failed to create DepthStencil");
		return false;
	}
	...
	{
		assert(0 && "Failed to create ShadowMap");
		return false;
	}
	...
	{
		assert(0 && "Failed to create SwapChain RTV");
		return false;
	}
	if (!CreateFence())
	{
		assert(0 && "Failed to create Fence");
		return false;
	}
