        ...
        [](ECSCoordinator& ecs, Entity e, const nlohmann::json& cj, GameObject* obj) {
            ...
            if (cj.contains("FilePath")) {
                ...
                if (!d.m_filePath.empty()) {
                    // Dynamic (skinned/animated) models skip the aiProcess_OptimizeGraph merge so that
                    // mesh-less named nodes (e.g. hand-attach sockets) survive import intact.
                    bool mergeMeshes = (d.m_modelType != ModelType::Dynamic);
                    d.m_spModelData = ResourceManager::Instance().LoadModelAsync(d.m_filePath, mergeMeshes);
                    // ResourceManager caches models by file path for the whole app lifetime (nothing
                    // ever clears it on scene change), so this can hand back the exact same ModelData
                    // instance a previous scene already animated - e.g. a door left mid-swing when
                    // Hunt ended and the scene changed to ResultScene. Node transforms live on that
                    // shared ModelData, not per-entity, so without resetting them here every fresh
                    // scene load kept rendering whatever pose the model was left in last time (doors
                    // stuck open, etc.) until something happened to play that exact animation again.
                    if (d.m_spModelData) {
                        d.m_spModelData->ResetAnimation();
                    }
                }
            }
            ecs.AddComponent(e, d);
