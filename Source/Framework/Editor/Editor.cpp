...
void Editor::DrawHierarchyAndInspector(Scene* scene) {
    ...
    if (ImGui::BeginTabBar("EditorControlTabs")) {
        if (ImGui::BeginTabItem("Scene")) {
            ...
            if (ImGui::Button("Game Scene", ImVec2(-1, 0))) {
                SceneManager::Instance().ChangeScene(std::make_shared<GameScene>(), 0.3f);
            }
            if (ImGui::Button("Title Scene", ImVec2(-1, 0))) {
                SceneManager::Instance().ChangeScene(std::make_shared<TitleScene>(), 0.3f);
            }
            ImGui::Separator();
    ...
    // ドラッグ＆ドロップで親から子へ、またはルート階層への移動処理
