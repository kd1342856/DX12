...
void CollisionManager::Solve(Scene* scene) {
    ...
    for (size_t i = 0; i < entitiesWithCollider.size(); ++i) {
        ...
        for (size_t j = i + 1; j < entitiesWithCollider.size(); ++j) {
            ...
            // è¿šãEå©¿ç¸ºç¢æ’•é€§ãƒ»ãƒ»è£E´èœ·åŒ»ãƒ»ç¸²âˆè™šé€§ãƒ»â†‘è­E½¹ç¸ºE½ç¸ºä»£E’èœè¼”Â°ç¸ºãƒ»
...
RaycastHit CollisionManager::RaycastAgainstMesh(const Math::Vector3& origin, const Math::Vector3& direction, float maxDistance, const std::string& targetName) {
    ...
    // èŸE½¾é›Žï½¡ç¹§Eªç¹æ‚¶ãšç¹§E§ç¹§E¯ç¹åŒ»â†’ç¸ºæ˜´ãƒ»èŸEEE’èœ€æ¦Šï½¸E°é€§ãƒ»â†“ç¹ï½¡ç¹ãEã™ç¹ï½¥è›»E¤èž³å£¹â˜E¹§ä¹Î›ç¹ï¿½ç¹Â€
    std::function<void(const std::shared_ptr<GameObject>&)> checkObj = [&](const std::shared_ptr<GameObject>& obj) {
        if (!obj || !obj->IsActive()) return;

        auto& ecs = GameManager::Instance().GetECS(); if (!ecs.HasComponent<ModelRenderData>(obj->GetEntityID()) || !ecs.HasComponent<TransformData>(obj->GetEntityID())) return;
        ...
        // èŸEEãŒç¹æ‚¶ãšç¹§E§ç¹§E¯ç¹åŒ»E‚èœ€æ¦Šï½¸E°é€§ãƒ»â†“è¬—ï½¢é‚ï½¢
        for (const auto& child : obj->GetChildren()) {
            checkObj(child);
        }
    };

    // ç¹§E·ç¹ï½¼ç¹ï½³èœ€ãƒ»ãƒ»ç¹§Eªç¹æ‚¶ãšç¹§E§ç¹§E¯ç¹åŒ»E’è¬—ï½¢é‚ï½¢
    auto& gameObjects = m_pCurrentScene->GetGameObjects();
    for (auto& obj : gameObjects) {
        if (!targetName.empty()) {
            // è¬–ãEE®å£¼éŒç¸ºE®ç¹§Eªç¹æ‚¶ãšç¹§E§ç¹§E¯ç¹èŒ¨E¼åŒ»â†’ç¸ºæ˜´ãƒ»èŸE’°E¼å³¨ãƒ»ç¸ºE¿ç¹âEã‰ç¹ãEãE
