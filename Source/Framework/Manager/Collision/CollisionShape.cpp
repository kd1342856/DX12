...
bool CollisionShapeMesh::RayCast(const RayInfo& ray, const Math::Matrix& world, RayResult& out) {
    ...
    for (const auto& node : m_model->GetNodes()) {
        Math::Matrix nodeLocal = node.animDeltaTransform;
...
void CollisionShapeMesh::UpdateWorldAABB(const Math::Matrix& world) {
    ...
    for (const auto& node : m_model->GetNodes()) {
        Math::Matrix nodeWorldMat = node.animDeltaTransform * worldMat;
        for (const auto& meshHandle : node.meshes) {
            auto mesh = MeshManager::Instance().Get(meshHandle);
            if (!mesh) continue;
            for (const auto& vert : mesh->GetVertices()) {
                Math::Vector3 wp = Math::Vector3::Transform(vert.Position, nodeWorldMat);
...
void CollisionShapeMesh::DrawDebug(const Math::Matrix& worldMatrix, uint32_t color) {
    // Mesh collider debug draw is too heavy to draw the full wireframe here, and its AABB is
    // already drawn separately (see the cyan box in CollisionManager::DrawDebugWires), so there's
    // nothing useful left to draw per-shape here.
    (void)worldMatrix;
    (void)color;
}
