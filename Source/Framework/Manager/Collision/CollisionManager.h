...
public:
    ...
    // シーン切り替え時に呼ぶ。静的コライダー用オクトツリーは一度構築したら
    // 二度と作り直されず、破棄済みのシーンが持っていたCollisionShapeへの
    // 生ポインタが残ったままになっていた(古いシーンのエンティティIDが新しいシーンで
    // 再利用されると、登録済み扱いされて再登録されずダングリングポインタのまま
    // 衝突判定に使われてしまい、クラッシュする原因になっていた)。
    // シーンをまたぐ前に必ずこれで静的コライダー関連の状態を全てクリアする。
    ...
    // navmesh visualization etc: bypasses the general debug-wire toggle, gated by its own caller instead
    void AddDebugLineAlways(const Math::Vector3& start, const Math::Vector3& end, ImU32 color);
    void AddDebugPolyAlways(const std::vector<Math::Vector3>& verts, ImU32 color);
...
private:
    ...
    std::vector<DebugPoly> m_debugPolys;
